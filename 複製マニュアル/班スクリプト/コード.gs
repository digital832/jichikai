...
function イベントを処理する(ラインイベント, ss, スプレッドシート, データベースシート, トークン, ファイル名, ファイル名からのグループ名) {
  ...
  if (ラインイベント.type === 'follow') {
    ...
    登録案内を返信(ラインイベント.replyToken, トークン, あいさつ文を作る(スプレッドシート), 登録リンクを作る(ss, ユーザーＩＤ), ユーザーＩＤ);
    console.log("登録リンクを返信しました（行" + 行番 + "）");
    return 新規;
  }

  // ==========================================
  // ➔ 通常のテキストメッセージ：基本は自動返信しない（管理者と住民のふだんのやり取り＝コミュニケーションの場として使うため）。
  //   名簿に行が無い人だけ行を作っておく。登録リンクは友だち追加時に送っている。
  //   ただし「登録」「リンク」「届かない」など、登録に困っている様子の短い一言が来たときだけ、登録案内を送り直す
  //   （案内が届かなかった人が、自分で再送を頼めるようにするため）。
  // ==========================================
  if (ラインイベント.type === 'message' && ラインイベント.message.type === 'text') {
    ...
    if (登録の再送依頼か(ラインイベント.message.text)) {
      登録案内を返信(ラインイベント.replyToken, トークン, あいさつ文を作る(スプレッドシート), 登録リンクを作る(ss, ユーザーＩＤ), ユーザーＩＤ);
      console.log("登録案内を再送しました（本人からの依頼）");
    }
    return 新規;
  }
  return false;
 ...
 */
function 登録案内を返信(返信トークン, トークン, 本文, リンク, ユーザーＩＤ) {
  const メッセージ一覧 = [];
  let ボタンの本文 = 本文;
  if (本文.length > 160) {
  ...
  // 返信トークンの期限切れなどで返信できなかった場合は、相手のIDへ直接送る（プッシュ）ことで案内が届かないままにならないようにする
  if (!返信を送る(返信トークン, トークン, メッセージ一覧) && ユーザーＩＤ) {
    プッシュを送る(ユーザーＩＤ, トークン, メッセージ一覧);
  }
}

// 「登録」「リンク」など、登録案内の再送を頼んでいると思われる短い一言か（登録完了の報告などは除く）
function 登録の再送依頼か(テキスト) {
  const 文 = String(テキスト || "").trim();
  if (文.length === 0 || 文.length > 15) return false;
  if (/しました|ました|済|ありがとう/.test(文)) return false;
  return /登録|リンク|届かな|届いて|できな|再送|ボタン/.test(文);
}

// LINEのプッシュAPI（返信トークンが使えないときの送り直し用）
function プッシュを送る(ユーザーＩＤ, トークン, メッセージ一覧) {
  try {
    const 応答 = UrlFetchApp.fetch("https://api.line.me/v2/bot/message/push", {
      "headers": { "Content-Type": "application/json; charset=UTF-8", "Authorization": "Bearer " + トークン },
      "method": "post",
      "payload": JSON.stringify({ "to": ユーザーＩＤ, "messages": メッセージ一覧 }),
      "muteHttpExceptions": true
    });
    if (応答.getResponseCode() !== 200) {
      console.log("プッシュに失敗（HTTP " + 応答.getResponseCode() + "）: " + 応答.getContentText());
      return false;
    }
    console.log("返信できなかったため、プッシュで登録案内を送りました");
    return true;
  } catch (e) {
    console.log("プッシュで例外: " + e);
    return false;
  }
}
